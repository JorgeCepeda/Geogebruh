...
public class EngineSwingASCIIKeyboard implements GFXEngine<JLabel> {
	private double[] pos;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(() -> {
			try {
				...
				EngineSwingASCIIKeyboard window = new EngineSwingASCIIKeyboard();
	 ...
	 */
	public EngineSwingASCIIKeyboard() {
		initialize();
	}
	...
	private void initialize() {
		...
		KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(e -> {
			if (e.getID() == KeyEvent.KEY_PRESSED && frmGFXEngine.getFocusOwner() == btnExecute) {
				...
				if (menu.isDisplayed()) menuHidden = menu.executeInput(e.getKeyCode());
				else {
					switch (e.getKeyCode()) {
					case KeyEvent.VK_M:
