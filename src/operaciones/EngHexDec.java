...
public class EngHexDec {
	...
	// Vector ordenado alfabéticamente
	private static final String[] colores = {"AliceBlue", "AntiqueWhite", "Aqua", "Aquamarine", "Azure", "Beige", "Bisque", "Black", "BlanchedAlmond", "Blue", "BlueViolet", "Brown",
	...
	public static String rgbToHex6(int[] rgb) {
		...
		for (int i = 0; i < colorDec.length; i++) {
			int cont = 0;
			int[] restos = new int[2];
			do {
				restos[cont] = colorDec[i] % 16;
				colorDec[i] /= 16;
				cont++;
			} while (colorDec[i] / 16 >= 1);
			restos[cont] = colorDec[i] % 16;
			
			for (int j = restos.length - 1; j >= 0; j--) {
				char carácter;
				switch (restos[j]) {
					case 15:
						carácter = 'F';
					...
					case 10:
						...
						break;
					default:
						if (restos[j] < 0) throw new IllegalArgumentException("RGB inválido: " + Arrays.toString(rgb));
						carácter = (char) (restos[j] + 48);
				}
	...
	public static int[] hex6ToRGB(String hexadecimal) {
		...
		if (hexadecimal.length() != 7) throw new IllegalArgumentException("Hexadecimal inválido: " + hexadecimal);
	...
	public static String engToHex6(String eng) {
		int izq = 0, der = colores.length - 1, swPos = -1;
		while (izq <= der) {
			int centro = (izq + der) / 2, res = comp.compare(colores[centro], eng);
			
			if (res == 0) {
				swPos = centro;
				break;
			}
			if (res < 0) izq = centro + 1;
			else der = centro - 1;
		}
		if (swPos < 0) throw new IllegalArgumentException("Color no encontrado: " + eng);
		return "#" + hex[swPos]; 
	}
